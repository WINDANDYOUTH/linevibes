...
import {
  getBasePaymentProviderId,
  getCheckoutPaymentSelectionKey,
  getPaypalCardOptionId,
  getPaypalWalletOptionId,
  isPaypal,
  isPaypalCardOption,
  isStripeLike,
  paymentInfoMap,
...
const Payment = ({
  ...
  const displayPaymentMethods = useMemo(() => {
    return availablePaymentMethods.flatMap((paymentMethod) => {
      if (!isPaypal(paymentMethod.id)) {
      ...
      return [
        {
          ...paymentMethod,
          id: getPaypalCardOptionId(paymentMethod.id),
        },
        {
          ...paymentMethod,
          id: getPaypalWalletOptionId(paymentMethod.id),
        },
      ]
    })
  }, [availablePaymentMethods])

  const persistPaymentSelection = useCallback(
    (method: string) => {
      if (!paymentSelectionStorageKey || typeof window === "undefined") {
        return
      }

      window.sessionStorage.setItem(paymentSelectionStorageKey, method)
    },
  ...
  const setPaymentMethod = async (method: string) => {
    ...
    const providerId = getBasePaymentProviderId(method) || method

    setError(null)
    setSelectedPaymentMethod(method)
    persistPaymentSelection(method)

    const requiresSession = isStripeLike(providerId) || isPaypal(providerId)

    if (!requiresSession) {
      return
    }

    if (activeSession?.provider_id === providerId) {
      return
    }
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      const requiresSession = isStripeLike(providerId) || isPaypal(providerId)
      const hasMatchingSession = activeSession?.provider_id === providerId
  ...
  useEffect(() => {
    const storedSelection =
      paymentSelectionStorageKey && typeof window !== "undefined"
        ? window.sessionStorage.getItem(paymentSelectionStorageKey)
        : null

    if (
      storedSelection &&
      getBasePaymentProviderId(storedSelection) === activeSession?.provider_id
    ) {
      setSelectedPaymentMethod(storedSelection)
      return
    }

    if (activeSession?.provider_id) {
      const defaultSelection = isPaypal(activeSession.provider_id)
        ? getPaypalWalletOptionId(activeSession.provider_id)
        : activeSession.provider_id

      setSelectedPaymentMethod(defaultSelection)
      persistPaymentSelection(defaultSelection)
    }
  }, [activeSession?.provider_id, paymentSelectionStorageKey, persistPaymentSelection])
  ...
  const paymentActionDisabled =
    ...
    (isStripeLike(selectedProviderId) && !cardComplete) ||
    (!selectedPaymentMethod && !paidByGiftcard) ||
    ((isStripeLike(selectedProviderId) || isPaypal(selectedProviderId)) &&
    ...
    <div className="checkout-section-wrapper">
      ...
      <div>
        <div className={isOpen ? "block" : "hidden"}>
          {!paidByGiftcard && displayPaymentMethods?.length && (
            <>
          ...
          >
            {isInitializingSession
              ...
              : !activeSession && isStripeLike(selectedProviderId)
                ? "Enter card details"
                : "Continue to review"}
          </Button>
        </div>
            ...
            <div className="flex w-full items-start gap-x-1">
              ...
              <div className="flex w-1/3 flex-col">
                ...
                >
                  ...
                  <Text>
                    {isStripeLike(selectedProviderId) && cardBrand
                      ? cardBrand
                      : isPaypalCardOption(selectedPaymentMethod)
                        ? "Card details will be charged directly"
